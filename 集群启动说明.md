# Hadoop 集群一键启动说明

## 📋 前置条件

确保以下镜像已存在：
- `myhadoop-namenode:v1`
- `myhadoop-datanode1:v1`
- `myhadoop-datanode2:v1`

检查镜像：
```bash
docker images | grep myhadoop
```

## 🚀 一键启动集群

### 启动命令
```bash
docker-compose up -d
```

### 查看状态
```bash
# 查看容器状态
docker-compose ps

# 查看日志
docker-compose logs -f namenode
```

### 停止集群
```bash
docker-compose down
```

### 停止并删除数据卷（会删除 HDFS 数据）
```bash
docker-compose down -v
```

## 🔌 端口映射

### NameNode
- **9870**: NameNode Web UI - http://localhost:9870
- **9000**: HDFS RPC
- **8020**: HDFS RPC (alternative)
- **50070**: NameNode Web UI (Hadoop 2.x)
- **2222**: SSH

### DataNode1
- **9864**: DataNode Web UI - http://localhost:9864
- **2223**: SSH

### DataNode2
- **9865**: DataNode Web UI - http://localhost:9865
- **2224**: SSH

## 📝 使用说明

### 1. 首次启动后格式化 NameNode（如果需要）
```bash
docker exec -it hadoop-namenode /bin/bash
su - hadoop
hdfs namenode -format
```

### 2. 启动 HDFS 服务
```bash
# 进入 namenode 容器
docker exec -it hadoop-namenode /bin/bash
su - hadoop

# 启动 HDFS
start-dfs.sh

# 检查状态
jps
hdfs dfsadmin -report
```

### 3. 测试 HDFS
```bash
# 在 namenode 容器中（hadoop 用户）
hdfs dfs -mkdir /test
hdfs dfs -ls /
hdfs dfs -put <本地文件> /test/
hdfs dfs -ls /test
```

### 4. 访问 Web UI
- NameNode: http://localhost:9870
- DataNode1: http://localhost:9864
- DataNode2: http://localhost:9865

## 🔧 常用操作

### 进入容器
```bash
# NameNode
docker exec -it hadoop-namenode /bin/bash

# DataNode1
docker exec -it hadoop-datanode1 /bin/bash

# DataNode2
docker exec -it hadoop-datanode2 /bin/bash
```

### 切换到 hadoop 用户
```bash
su - hadoop
```

### 查看集群状态
```bash
# 在 namenode 容器中（hadoop 用户）
hdfs dfsadmin -report
jps
```

### 查看日志
```bash
# NameNode 日志
docker exec hadoop-namenode tail -f /usr/local/hadoop/logs/hadoop-hadoop-namenode-namenode.log

# DataNode 日志
docker exec hadoop-datanode1 tail -f /usr/local/hadoop/logs/hadoop-hadoop-datanode-datanode1.log
```

## ⚠️ 注意事项

1. **数据持久化**：HDFS 数据保存在 Docker volumes 中，删除容器不会丢失数据
2. **SSH 访问**：使用映射的端口访问 SSH（2222, 2223, 2224）
3. **网络**：所有容器在 `hadoop-network` 网络中，可以通过 hostname 互相访问
4. **重启策略**：容器设置为 `unless-stopped`，会自动重启

## 🐛 故障排查

### 容器无法启动
```bash
# 查看日志
docker-compose logs namenode
docker-compose logs datanode1
docker-compose logs datanode2
```

### 端口冲突
如果端口被占用，修改 `docker-compose.yml` 中的端口映射

### 网络问题
```bash
# 检查网络
docker network inspect hadoop-network

# 重新创建网络
docker-compose down
docker network rm hadoop-network
docker-compose up -d
```

